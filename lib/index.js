...
function () {
  ...
  function TendermintClientWS(options) {
    ...
    value: function connect(options) {
      ...
      this.$eventsSubscription = new Observable(function (observable) {
        ...
        var openHandler = function openHandler() {
          _this.isSynced = true;
          console.log('Connected to node web socket'); // Handle subscribe keys

          options.subscribeTo.forEach(function (key) {
            _this.subscribe(key);
          });
        }; // when socket closed
...
function (_TendermintClientWS) {
  ...
  function TendermintClient(options) {
    ...
    _this2 = _possibleConstructorReturn(this, _getPrototypeOf(TendermintClient).call(this, options));
